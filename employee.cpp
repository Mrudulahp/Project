...
void main()
{
 ...
 do
 {
  ...
  switch(option)
  {
   case '1':
	    ...
	    while(!file.eof())
	    {
	     ...
	     {
	       cout<<"This ID alreadt exist.. try for another ID";
	       isFound=1;
	       break;
	     }
	     file.read((char*)&e,sizeof(e));
  ...
  case '2':
	    isFound=0;
	    cout<<endl<<"enter i=ID of an employee to be searched:";
	    cin>>ID;
	    file.seekg(0,ios::beg);
	    file.read((char*)&e,sizeof(e));
	    while(!file.eof())
 ...
 case '5':
	   int recordNo=0;
	   cout<<endl<<"file if being modifief..";
	   ...
	   while(!file.eof())
	   {
	    ...
	    cout<<endl<<"enter the new record for employee having ID"<<ID;
	    e.ReadData();
